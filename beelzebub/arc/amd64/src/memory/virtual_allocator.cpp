...
//  Used to map pages prior to having an active 
Handle MapEx(VirtualAllocationSpace * const vas, const vaddr_t vaddr, const paddr_t paddr, const PageFlags flags)
{
    if ((vaddr >= VirtualAllocationSpace::FractalStart && vaddr < VirtualAllocationSpace::FractalEnd     )
     || (vaddr >= VirtualAllocationSpace::LowerHalfEnd && vaddr < VirtualAllocationSpace::HigherHalfStart))
        return Handle(HandleResult::PageMapIllegalRange);

    if ((vaddr & 0xFFFULL) != 0 || (paddr & 0xFFFULL) != 0)
        return Handle(HandleResult::PageUnaligned);

    uint16_t ind;   //  Used to hold the current index.

    Pml4 & pml4 = *((Pml4 *)vas->Pml4Address);
    ind = VirtualAllocationSpace::GetPml4Index(vaddr);
    ...
    if (!pml4[ind].GetPresent())
    {
        ...
        const paddr_t newPml3 = vas->Allocator->AllocatePage();
    ...
    Pml3 & pml3 = *(pml4[ind].GetPml3Ptr());
    ind = VirtualAllocationSpace::GetPml3Index(vaddr);
    ...
    if (!pml3[ind].GetPresent())
    {
        ...
        const paddr_t newPml2 = vas->Allocator->AllocatePage();
    ...
    Pml2 & pml2 = *(pml3[ind].GetPml2Ptr());
    ind = VirtualAllocationSpace::GetPml2Index(vaddr);
    ...
    if (!pml2[ind].GetPresent())
    {
        ...
        const paddr_t newPml1 = vas->Allocator->AllocatePage();
    ...
    Pml1 & pml1 = *(pml2[ind].GetPml1Ptr());
    ind = VirtualAllocationSpace::GetPml1Index(vaddr);
