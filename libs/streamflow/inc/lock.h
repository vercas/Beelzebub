...
#pragma once
    ...
    #define STREAMFLOW_LOCK_WITH_COOKIES

    #include <beel/sync/spinlock.unint.h>

    typedef Spinlock lock_t;
    typedef SpinlockUninterruptibleCookie lock_cookie_t;

    static __forceinline void spin_init(lock_t * const lock)
    {
        SpinlockUninterruptibleReset(lock);
    }
    static __forceinline __must_check lock_cookie_t spin_lock(lock_t * const lock)
    {
        return SpinlockUninterruptibleAcquire(lock);
    }
    static __forceinline void spin_unlock(lock_t * const lock, lock_cookie_t const cookie)
    {
        SpinlockUninterruptibleRelease(lock, cookie);
    }
#else
    #include "atomic.h"
