...
static int radix_register(void* start, int num_pages, void* ptr, size_t size, short object_type);
...
static int get_free_superpage(superpage_t** sp, size_t size);
static void* supermap(size_t size);
static void superunmap(void* start, size_t length);

/* All virtual page operations. */
static inline int register_pages(void* start, int num_pages, void* ptr, size_t size, short object_type);
...
static inline unsigned int quick_log2(unsigned int x)
{
    ...
    DebugMessage("quick_log2() unhandled number: %u4%n", x);
    assert(0);
...
static int radix_register(void* start, int num_pages, void* ptr, size_t size, short object_type)
{
    ...
    {
        unsigned long const level1 = page >> (RADIX_INTERIOR_BITS + RADIX_LEAF_BITS);
        unsigned long const level2 = (page >> RADIX_LEAF_BITS) & (RADIX_INTERIOR_SIZE - 1);
        unsigned long const level3 = page & (RADIX_LEAF_SIZE - 1);
        page_record_t record;

        // DebugMessage("radix_register: S %Xp P %Xp 1 %Xp 2 %Xp 3 %Xp%n", start, page, level1, level2, level3);

        if (radix_root->prefixes[level1] == NULL)
        {
            radix_interior_t* temp_interior = radix_interior_alloc();

            if (unlikely(temp_interior == NULL))
                return 0;
        ...
        case OBJECT_SMALL:
#ifdef __BEELZEBUB_KERNEL
            record.pageblock = ((unsigned long)ptr & 0x0000FFFFFFFFFFFFUL) >> PAGE_BITS;
#else
        ...
        case OBJECT_MEDIUM:
#ifdef __BEELZEBUB_KERNEL
            record.sph = ((unsigned long)ptr & 0x0000FFFFFFFFFFFFUL) >> SUPERPAGE_BITS;
#else
...
{
#ifdef __BEELZEBUB_KERNEL
    unsigned long const page = ((unsigned long)object & 0x0000FFFFFFFFFFFFUL) >> PAGE_BITS;
#else
    unsigned long const page = (unsigned long)object >> PAGE_BITS;
#endif
...
#ifdef __BEELZEBUB_KERNEL
    if ((unsigned long)(*ptr) >> 47)
        // *((unsigned long *)ptr) |= 0xFFFF000000000000UL;
        *ptr = (void *)((unsigned long)(*ptr) | 0xFFFF000000000000UL);

    // DebugMessage("radix_extract: %X2 %Xp %Xs for %Xp%n", *object_type, *ptr, *size, object);
#endif
}
...
static void* quickie_alloc(quickieblock_t* quickie, size_t object_size)
{
    ...
    if (quickie->unallocated == NULL || quickie->num_free_objects == 0)
    {
        ...
        if (unlikely(quickie->unallocated == NULL))
            return NULL;
    ...
    else
    {
        ...
        quickie->unallocated = (void *)((uintptr_t)quickie->unallocated + object_size);
    }
...
static int get_free_superpage(superpage_t** super, size_t size)
{
    ...
    *super = NULL;
    ...
    if (curr != NULL)
    {
        ...
        do
        {
            ...
            if (((superpage_t*)curr)->largest_free_order < BUDDY_ORDER_MAX && 
                (1UL << ((superpage_t*)curr)->largest_free_order) >= (size / PAGE_SIZE))
            {
    ...
    if (unlikely(*super == NULL))
    {
        *super = (superpage_t*)quickie_alloc(&sph_pageblocks, sizeof(superpage_t));
        ...
        if (unlikely((*super)->page_pool == NULL))
            return 0;
        ...
        {
            byte += max(sizeof(unsigned long), DivRoundUp32(PAGES_PER_SUPERPAGE, (1 << order) * 8 * 2));
...
#ifdef STREAMFLOW_LOCK_WITH_COOKIES
    lock_cookie_t const cookie = spin_lock(&super_lock);
#else
    spin_lock(&super_lock);
#endif

    if (unlikely(!get_free_superpage(&super, size)))
        return NULL;
...
// #endif
#ifdef STREAMFLOW_LOCK_WITH_COOKIES
    lock_cookie_t const cookie = spin_lock(lock);
...
{
#ifdef RADIX_TREE
    return radix_register(start, num_pages, ptr, size, object_type);
// #elif BIBOP
//     int i;
...
static inline void* page_alloc(size_t size)
{
    ...
    if (addr == MAP_FAILED) {
        // DebugMessage("page_alloc() mmap of size %us failed%n", size);
        // exit(1);
        return NULL;
    }
...
static void* medium_or_large_alloc(size_t size)
{
    ...
    if (size <= SUPERPAGE_SIZE) {
        ...
        unsigned int pow = FastCeilLog2_S(size);
        size = 1 << pow;
        mem = supermap(size);
        ...
        if (unlikely(!register_pages(mem, 1, ((pageblock_t*)mem)->sph, size, OBJECT_MEDIUM)))
            return NULL;
        ...
        if (unlikely(!register_pages(mem, 1, NULL, size, OBJECT_LARGE)))
            return NULL;
...
static inline int compute_pageblock_size(int index)
{
    ...
    unsigned int suggestion = RoundUpS(reverse_size_class(index) * OBJECTS_PER_PAGEBLOCK, PAGE_SIZE);

    // 2^pow is the closest power-of-2 to suggestion.
    unsigned int const pow = FastCeilLog2_S(suggestion + suggestion / 2);

    // Make sure that suggestion is a power-of-2.
    suggestion = 1 << pow;

    if (unlikely(suggestion < MIN_PAGEBLOCK_SIZE))
        return MIN_PAGEBLOCK_SIZE;
    if (unlikely(suggestion > MAX_PAGEBLOCK_SIZE))
...
static pageblock_t* get_free_pageblock(heap_t* heap, int index)
{
    ...
    if (pageblock == NULL) {
        ...
        if (unlikely(pageblock == NULL))
            return NULL;

        if (unlikely(!register_pages(pageblock, pageblock_size / PAGE_SIZE, pageblock, 0, OBJECT_SMALL)))
            return NULL;
        ...
        pageblock->mem_pool = (char*)((unsigned long)pageblock + RoundUpS(sizeof(pageblock_t), CACHE_LINE_SIZE));
...
// {
//  DebugMessage("totsmall %u totmedium %u totlarge %u szsmall %llu szmedium %llu szlarge %llu frees %u remote %u adopt %u\n",
...
static inline void memory_init_check(void)
{
    ...
    {
#ifdef STREAMFLOW_LOCK_WITH_COOKIES
        lock_cookie_t const cookie = spin_lock(&init_lock);
#else
        spin_lock(&init_lock);
#endif
...
void* valloc(size_t size)
{
    ...
    DebugMessage("valloc() called. Not implemented! Exiting.%n");
    assert(0);
}

void* memalign(size_t const boundary, size_t const size)
{
    void *p;
...
void *realloc(void *object, size_t size)
{
    ...
    if (object_type == OBJECT_LARGE)
    {
        ...
        else if (size < old_size)
        {
            ...
            // Remap without allowing virtual address change.

            new_object = mremap(object, old_size, size, 0);

            if (new_object == MAP_FAILED)
            {
                DebugMessage("realloc(): first mremap failed at address %Xp for %us -> %us%n", object, old_size, size);
                assert(0);
            }
        }
        else
        {
            ...
            if (new_object == MAP_FAILED)
            {
                DebugMessage("realloc(): second mremap failed at address %Xp for %us -> %us%n", object, old_size, size);
                assert(0);
            }
        }
        ...
        if (unlikely(!register_pages(new_object, 1, NULL, size, OBJECT_LARGE)))
            return NULL;
    ...
    else if (object_type == OBJECT_MEDIUM)
    {
        ...
        else if (size > MAX_OBJECT_SIZE && size <= SUPERPAGE_SIZE)
        {
            ...
            size_t super_size = RoundUpS(size, PAGE_SIZE); // ceil((double)size / PAGE_SIZE) * PAGE_SIZE;
            unsigned int pow = FastCeilLog2_S(super_size);
            super_size = 1 << pow;
            ...
            if (unlikely(!register_pages(new_object, 1, ((pageblock_t*)new_object)->sph, super_size, OBJECT_MEDIUM)))
                return NULL;
